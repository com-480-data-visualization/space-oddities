...
const CAR_W = 54, CAR_H = 22
const PX_PER_M = CAR_H / CAR_LENGTH_M
...
const LEFT_EDGE  = CAR_W / 2 + 16
const RIGHT_EDGE = W - CAR_W / 2 - 16

// Custom easing: slows down in the central ~3 s so the near-miss is dramatic.
// progress is raw time 0→1; returns visual position 0→1.
function ease(t) {
  // Split: 0–0.35 → 0–0.45 (fast approach)
  //        0.35–0.65 → 0.45–0.55 (slow, near TCA)
  //        0.65–1   → 0.55–1    (fast departure)
...
function CarTopDown({ cx, cy, fill, facingRight = true }) {
  ...
  return (
    <g>
      <rect x={cx - hw} y={cy - hh} width={CAR_W} height={CAR_H} rx={3}
...
export default function CarAnimationMiss({ cdm, approachData, mode = 'miss', onProgress }) {
  const [rawProgress, setRawProgress] = useState(0)   // 0→1 linear in time
  ...
  // Compute the gap to show at TCA
  ...
  // CDM miss distance (chapter 6 label)
  ...
  useEffect(() => {
    ...
    const step = ts => {
      ...
      setRawProgress(t)
      onProgressRef.current?.((ease(t) - 0.5) * 40)  // map p→[-20, +20] minutes
      if (t < 1) {
  ...
  // Visual position p (0→1 with TCA at p=0.5)
  const p = ease(rawProgress)

  // Car X positions — cross at centre at p=0.5
  const car1CX = LEFT_EDGE  + p * (RIGHT_EDGE - LEFT_EDGE)   // → (left to right)
  const car2CX = RIGHT_EDGE - p * (RIGHT_EDGE - LEFT_EDGE)   // ← (right to left)

  // Y gap from the chosen distance
  const scaledGapM  = gapAtTCA != null ? gapAtTCA * SCALE : null
  const maxGapPx    = ROAD_H * 0.65 - CAR_H
  const gapPx       = scaledGapM != null
    ? Math.min(Math.max(scaledGapM * PX_PER_M, 0.5), maxGapPx)
    : 8
  const laneOffset  = gapPx / 2 + CAR_H / 2
  const car1CY      = ROAD_CY + laneOffset
  const car2CY      = ROAD_CY - laneOffset

  // Show the gap indicator only near TCA (p 0.4–0.6)
    ...
    <div className="cam-wrap">
      ...
      <div className="cam-svg-wrap">
        <svg viewBox={`0 0 ${W} ${H}`} className="cam-svg">
          ...
          {gapOpacity > 0.05 && car1InnerY > car2InnerY + 1 && (
          ...
          {isMiss ? (
            <>
              ...
              <text x={W / 2} y={infoTop + 36} textAnchor="middle" fontSize={16} fill="#374151"
                fontFamily='"Space Grotesk", sans-serif'>→</text>
              <text x={W / 2} y={infoTop + 50} textAnchor="middle" fontSize={7.5} fill="#374151"
                fontFamily='"Space Mono", monospace'>at 1 : 315 scale</text>
              ...
              <text x={W - 20} y={infoTop + 34} textAnchor="end" fontSize={14} fontWeight={700}
                fill={scaledGapM != null && scaledGapM < 0.05 ? '#ef4444' : '#fb923c'}
                fontFamily='"Space Grotesk", sans-serif'>
                {gapLabel}
              </text>
              {scaledCdmM != null && scaledCdmM < 0.05 && (
            ...
            <>
              ...
              <text x={W / 2} y={infoTop + 38} textAnchor="middle" fontSize={7.5} fill="#ef444466"
                fontFamily='"Space Mono", monospace'>
                ↑ {cdmMissM != null ? Math.round(Math.min(...approachData.map(p => p.distM)) / cdmMissM) : '?'}× off
              </text>
              <text x={W - 20} y={infoTop + 16} textAnchor="end" fontSize={9} fill="#64748b"
                fontFamily='"Space Grotesk", sans-serif' fontWeight={600}>
